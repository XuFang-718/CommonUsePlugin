...
public class BestGamemode implements CommandExecutor {
    ...
    public boolean onCommand(@NotNull CommandSender sender, @NotNull Command command, @NotNull String label, @NotNull String[] args) {
        ...
        if (args.length == 0) {
            ...
            ChatUtil.pluginSay(player, PREFIX, "&b你的模式是 &6" + gameMode.name());
            return true;
        }

        if ("0".equalsIgnoreCase(args[0])) {
            player.setGameMode(GameMode.SURVIVAL);
            ChatUtil.pluginSay(player, PREFIX, CU.t("&r您已成为&6 生存模式"));
        }

        if ("1".equalsIgnoreCase(args[0])) {
            player.setGameMode(GameMode.CREATIVE);
            ChatUtil.pluginSay(player, PREFIX, CU.t("&r您已成为&6 创造模式"));
        }

        if ("2".equalsIgnoreCase(args[0])) {
            player.setGameMode(GameMode.ADVENTURE);
            ChatUtil.pluginSay(player, PREFIX, CU.t("&r您已成为&6 冒险模式"));
        }

        if ("3".equalsIgnoreCase(args[0])) {
            player.setGameMode(GameMode.SPECTATOR);
            ChatUtil.pluginSay(player, PREFIX, CU.t("&r您已成为&6 旁观者模式"));
        }
        return true;
    }
}
