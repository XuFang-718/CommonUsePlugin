...
/**
 * TPA Here 命令 - /tpahere <玩家名>
 * 请求目标玩家传送到自己身边
 */
public class TpaHereCommand implements CommandExecutor, TabExecutor {

    // 目标玩家UUID -> (请求者名字 -> 请求者UUID)
    public static HashMap<UUID, HashMap<String, UUID>> TpaHereRequests = new HashMap<>();
    // 目标玩家UUID -> (请求者UUID -> 超时任务)
    public static HashMap<UUID, HashMap<UUID, BukkitTask>> TpaHereTasks = new HashMap<>();
    ...
    public boolean onCommand(@NotNull CommandSender commandSender, @NotNull Command command, @NotNull String s, @NotNull String[] strings) {
        ...
        if (strings.length == 0) {
            player.sendMessage(buildPrefix().append(Component.text("用法: ", NamedTextColor.GRAY))
                    .append(Component.text("/tpahere <玩家名>", NamedTextColor.YELLOW)));
            return false;
        }
        ...
        if (target.equals(player)) {
            player.sendMessage(buildPrefix().append(Component.text("不能请求自己传送!", NamedTextColor.RED)));
            return false;
        }
        ...
        TpaHereRequests.computeIfAbsent(targetUUID, k -> new HashMap<>());
        TpaHereTasks.computeIfAbsent(targetUUID, k -> new HashMap<>());

        if (TpaHereRequests.get(targetUUID).containsValue(playerUUID)) {
        ...
        TpaHereRequests.get(targetUUID).put(playerName, playerUUID);

        HashMap<UUID, BukkitTask> targetTasks = TpaHereTasks.get(targetUUID);
        ...
        BukkitTask task = Bukkit.getScheduler().runTaskLater(CommonUsePlugin.instance, () -> {
            HashMap<String, UUID> requests = TpaHereRequests.get(targetUUID);
            ...
            HashMap<UUID, BukkitTask> tasks = TpaHereTasks.get(targetUUID);
            if (tasks != null) {
                tasks.remove(playerUUID);
                if (tasks.isEmpty()) {
                    TpaHereTasks.remove(targetUUID);
                }
            }
        ...
        // 发送给目标玩家
        sendTpaHereRequest(target, playerName);
     ...
     * 发送美化的 TPA Here 请求消息
     */
    private void sendTpaHereRequest(Player target, String requesterName) {
        target.sendMessage(Component.text("━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━", NamedTextColor.LIGHT_PURPLE));
        ...
        target.sendMessage(Component.text("  玩家 ", NamedTextColor.GRAY)
                ...
                .append(Component.text(" 希望您传送到TA身边", NamedTextColor.GRAY)));

        target.sendMessage(Component.empty());

        Component acceptButton = Component.text("  [✔ 同意]", NamedTextColor.GREEN, TextDecoration.BOLD)
                .clickEvent(ClickEvent.runCommand("/tphereaccept " + requesterName))
                .hoverEvent(HoverEvent.showText(Component.text("点击同意传送到 " + requesterName + " 身边", NamedTextColor.GREEN)));

        Component denyButton = Component.text("  [✘ 拒绝]", NamedTextColor.RED, TextDecoration.BOLD)
                .clickEvent(ClickEvent.runCommand("/tpheredeny " + requesterName))
